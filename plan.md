...
- **산출물**: 파라미터 민감도 테이블 (`scripts/sensitivity.py` CLI, CSV/Markdown 출력) — 완료. 단일 런 백테스트 CLI (`scripts/backtest.py`) — 완료. KIS 과거 분봉 API 어댑터 (`data/kis_minute_bars.py`, ADR-0016) — 완료 2026-04-22. 백테스트 리포트 HTML/노트북은 Phase 5 후보 (의도적 defer). **Phase 2 PASS 선언 잔여 조건**: 1년치 KIS 분봉 백필 (`--loader=kis --from=<1년 전> --to=<오늘>`) + `uv run python scripts/backtest.py --loader=kis --csv-dir ... --from <1년 전> --to <오늘>` 실행 후 낙폭 절대값 15% 미만 (MDD > -15%) 확인. 표본 기간 240 영업일 이상·다중 종목 필수 (ADR-0017). Issue #36 close 예정.
...
- **Phase 2**:
  ...
  - `python scripts/backtest.py --from <1년 전 날짜> --to <오늘>` → 리포트 생성, 수수료·세금 반영, 낙폭 절대값 15% 미만 (MDD > -15%). 표본 기간: 연속 최소 240 영업일 (약 1년, KIS 서버 1년 보관 한도, ADR-0017). 단일 종목 단독 PASS 근거 불충분 — 다중 종목 실행이 공식 선언 조건.

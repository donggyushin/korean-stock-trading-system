...
**Phase 2 진행 중 — ORB 전략 엔진 + 리스크 매니저 + 백테스트 엔진 코어 + CSV 분봉 어댑터 + 파라미터 민감도 그리드 + backtest.py CLI + KIS 과거 분봉 API 어댑터 완료** (2026-04-20~22). `strategy/` + `risk/` + `backtest/` + `data/kis_minute_bars.py` 완료. `scripts/backtest.py`·`scripts/sensitivity.py` 에 `--loader={csv,kis}` 옵션 추가. **PASS 선언은 실데이터 수집 후 낙폭 절대값 15% 미만 확인 (MDD > -15%)까지 보류** — 2~3년 분봉 CSV 확보(운영자 외부 작업) 필요. **제한 사항**: KIS 서버는 과거 분봉을 최대 1년만 보관한다. 2~3년 백테스트 기간은 `--loader=kis` 로 해결 불가 — CSV 경로 또는 별도 데이터 소스 필요 (Issue #5). 상세 설계와 각 Phase의 PASS 기준, 비용·위험 분석은 [`plan.md`](./plan.md)에 있습니다.

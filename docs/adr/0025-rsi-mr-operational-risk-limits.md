...
- 코드 (PR4 완료, 2026-05-03): `src/stock_agent/execution/executor.py` — `BarSource` Protocol 에 `subscribe/unsubscribe` 추가 + `_handle_entry`/`_handle_exit`/`restore_session` 동적 구독 연동. `src/stock_agent/main.py` — `_build_runtime` universe 사전 구독 루프 제거. (`src/stock_agent/data/realtime.py` 변경 없음 — `subscribe/unsubscribe` 기존 공개 API 재사용)
...
- 도입 PR: PR3 (머지 완료, #111, 2026-05-03) · PR4 (머지 완료, 2026-05-03).

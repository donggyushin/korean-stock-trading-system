...
- 도입 PR: TBD (Issue #35).
- [2026-04-23] Issue #52 대응: `_parse_row` 원인 카테고리별 dedupe 로깅 + M2 경보에 첫 행 `sorted(keys)` 동봉 (PR <TBD>). 카테고리: `missing_date_or_time` / `date_mismatch` / `invalid_price` / `invalid_volume` / `malformed_bar_time`. 진단 스크립트 `scripts/debug_kis_minute.py` 신규. `scripts/backfill_minute_bars.py` `--throttle-s` 기본값 0.0 → 0.2 (KIS 실전 시세 API 20 req/s 한도 대비 5 req/s 수준 유지). 실 응답 키 계약 최종 확정은 장중 운영자 실행 결과 후속.

...
| 5 | `below_min_notional` | 산출 명목 < `config.min_notional_krw` |
| 6 | `insufficient_cash` | 산출 명목 > `available_cash_krw` |
| — | (승인) | 위 조건 모두 미해당 |
...
| 예외 유형 | 발생 조건 | 예외 클래스 |
|---|---|---|
| 사용자 입력 오류 | 세션 미시작 상태에서 호출, naive datetime, symbol 6자리 위반, 음수 qty/price/cash | `RuntimeError` |
| 상태 머신 무결성 오류 | 미보유 심볼 청산 시도, 동일 심볼 중복 체결 기록 | `RiskManagerError` |

`RiskManagerError` 는 `logger.exception` 동반, 원본 예외는 `__cause__` 로 보존.
...
- 관련 테스트 파일: `tests/test_risk_manager.py` (71 케이스).

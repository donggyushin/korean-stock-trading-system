...
Executor(
    *,
    symbols: tuple[str, ...],
    strategy: Strategy,          # ADR-0025 PR2: ORBStrategy → Strategy Protocol 로 확장
...
3. isinstance(strategy, ORBStrategy) 가 True 인 경우 (ADR-0025 PR2):
   ...
   else (RSIMRStrategy 등 일봉 전략):
   - ORB 상태 복원 루프 skip. 일봉 전략은 EOD 일봉 재흐름으로 자연 복원 가정 + warning 로그.
4. _halt = False, _last_reconcile = None 리셋.
```

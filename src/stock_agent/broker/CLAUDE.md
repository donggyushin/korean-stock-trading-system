...
- **`kis_client.py`** — 완료 (Phase 1 첫 산출물 + Phase 3 다섯 번째 산출물 확장)
  - 공개 API: `get_balance()`, `place_buy()`, `place_sell()`, `get_pending_orders()`, `cancel_order(order_number: str) -> None`, 보조로 `ensure_token()` 과 `close()`
  - 정규화 DTO: `BalanceSnapshot`, `OrderTicket`, `PendingOrder`, `Holding` (`@dataclass(frozen=True, slots=True)`)
    - `PendingOrder.qty_filled: int` 필드 추가 (ADR-0014). `_to_pending_order` 가 PyKis 정식 필드(`executed_quantity`/`pending_quantity`) 우선 매핑 → `qty_remaining` attribute fallback.
  - `cancel_order(order_number)` — 멱등, `OrderRateLimiter` 경유, `_call` 에러 래핑. KIS 취소 API 호출 + `KisClientError` 전파.
  ...
  - pytest 케이스로 공개 계약 잠금 (paper/live 분기, DTO 정규화, 에러 래핑, 컨텍스트 매니저, 가드 3종, cancel_order 멱등·에러 전파). 절대 건수는 root CLAUDE.md "현재 상태" 누적 카운트가 정본.

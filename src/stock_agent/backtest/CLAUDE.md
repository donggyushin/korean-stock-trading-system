...
`TradeRecord`, `DailyEquity`, `BarLoader`, `InMemoryBarLoader`
...
- **`BacktestResult`** — `trades` · `daily_equity` · `metrics` · `rejected_counts` (RejectReason 별 거부 횟수 — 부수 진단용).
...
2. 거부 → `rejected_counts[reason] += 1`.
3. 승인 → `entry_fill = buy_fill_price(signal.price, slippage)` 계산.
4. `notional_int + buy_commission > cash` 면 사후 거부 (`rejected_counts["insufficient_cash"] += 1`) — 슬리피지·수수료 반영 후 잔액 부족. RiskManager `entries_today` 미증가.

...
- **Phase 3 두 번째 산출물 — main.py + APScheduler 통합 (코드·테스트 레벨) 완료 (2026-04-21)**
  ...
  - 리소스 정리: SIGINT/SIGTERM → `_graceful_shutdown` → `scheduler.shutdown(wait=False)` → `realtime_store.close()` → `kis_client.close()`. `finally` 블록에서도 중복 호출(멱등).
  - pytest **605 → 652건 green** (`tests/test_main.py` 47건 신규). 회귀 0건. 의존성 추가: `apscheduler 3.11.2` + transitive `tzlocal 5.3.1`.
  - 미완료: `monitor/notifier.py` (텔레그램 알림) · `storage/db.py` (SQLite 영속화) · `broker/` 체결조회 API 통합 — 모두 후속 PR. **Phase 3 PASS 선언은 모의투자 연속 10영업일 무중단 운영 후.**
...
- **다음 작업**
  ...
  - **Phase 3 후속 PR**: `execution/executor.py` 완료 (2026-04-21) · `main.py` 완료 (2026-04-21) · `monitor/notifier.py` (텔레그램 알림, 미착수) · `storage/db.py` (SQLite 체결 기록, 미착수) 순서로 진행.

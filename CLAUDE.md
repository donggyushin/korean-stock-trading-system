...
- **Phase 2 진행 중 — ORB 전략 엔진 + 리스크 매니저 + 백테스트 엔진 코어 + CSV 분봉 어댑터 완료** (2026-04-20)
  ...
  - pytest **245 → 324 → 384 → 464 → 477건 green** (test_sensitivity 80 → 93건 보강: `SensitivityRow` 계약 10건 + `post_slippage_rejections` end-to-end + `engine.commission_rate`/`engine.sell_tax_rate` 라우팅 회귀 3건). 회귀 없음. 의존성 추가 없음.
  - 미완료: `scripts/backtest.py` CLI, KIS 과거 분봉 API 어댑터(별도 PR), 2~3년 실데이터 백테스트 PASS 검증(MDD < -15%). Phase 2 전체 PASS 선언은 이후.

- **다음 작업**
  - **Phase 2 잔여 (후속 PR)**: `scripts/backtest.py` CLI · KIS 과거 분봉 API 어댑터(별도 PR) · 2~3년 실데이터 백테스트 PASS 검증(MDD < -15%). (`scripts/backtest.py` 는 아직 미존재 — "실행 가능" 아님.)

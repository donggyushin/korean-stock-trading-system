...
Python 기반 한국주식 자동매매 시스템 (2026-05-03 기준 일중 데이트레이딩 가정 폐기 → 일/월 단위 평균회귀 채택 후보). 한국투자증권 KIS Developers API + 100~200만원 초기 자본. **paper 주문 + live 시세 하이브리드 키** 구조 (KIS paper 도메인에 시세 API 없음 — 시세는 별도 실전 APP_KEY 로 실전 도메인 호출). 현재 **Phase 1 PASS · Phase 2 PASS (2026-05-03 공식 선언)** — ADR-0023 의 C1~C4 추가 검증 전원 통과. F5 RSI 평균회귀 (`RSIMRStrategy`) 1차 채택 후보 확정 (ADR-0023). **Phase 3 착수 가능** — `main.py` 모의투자 무중단 운영 재허가. Phase 3 코드 산출물 (Executor·main.py APScheduler·monitor/notifier·storage/db·세션 재기동·broker 체결조회) 완료 상태 보존.
...
- 리스크 한도 (Phase 3 모의투자 운영 기준값, ADR-0025): `RiskConfig` — position_pct=0.10 (세션 자본의 10%), max_positions=10, daily_loss_limit_pct=-2% (서킷브레이커 보존), daily_max_entries=5. `RSIMRConfig` — stop_loss_pct=3%, max_positions=10, position_pct=1.0 (백테스트 자금 배분 의미, 운영 RiskConfig 와 다름). force_close_at 운영 미사용 (일봉 전략 특성상). ORB 시절 기본값(position_pct 20%, max_positions 3, daily_max_entries 10)은 ADR-0025 로 대체됨.
...
- [src/stock_agent/strategy/CLAUDE.md](./src/stock_agent/strategy/CLAUDE.md) — 전략 엔진 모듈 (Strategy Protocol, EntrySignal/ExitSignal DTO + ORBStrategy/StrategyConfig + VWAPMRStrategy/VWAPMRConfig — ADR-0019 Step E PR2 + GapReversalStrategy/GapReversalConfig — Step E PR3 + DCAStrategy/DCAConfig — Step F PR1 + GoldenCrossStrategy/GoldenCrossConfig — Step F PR2 + MomentumStrategy/MomentumConfig — Step F PR3 + LowVolStrategy/LowVolConfig — Step F PR4 + RSIMRStrategy/RSIMRConfig — Step F PR5)
...
- [src/stock_agent/backtest/CLAUDE.md](./src/stock_agent/backtest/CLAUDE.md) — 백테스트 엔진 모듈 (BacktestEngine, BacktestConfig, BacktestResult, BacktestMetrics, TradeRecord, DailyEquity, BarLoader, InMemoryBarLoader; 자체 시뮬레이션 루프, 한국 시장 비용 반영) + 민감도 그리드 (ParameterAxis, SensitivityGrid, SensitivityRow, run_sensitivity, run_sensitivity_combos, run_sensitivity_combos_parallel, render_markdown_table, write_csv, default_grid, step_d1_grid, step_d2_grid, append_sensitivity_row, load_completed_combos) + DCA baseline 평가 (DCABaselineConfig, compute_dca_baseline — Step F PR1) + Golden Cross baseline 평가 (GoldenCrossBaselineConfig, compute_golden_cross_baseline — Step F PR2) + Momentum baseline 평가 (MomentumBaselineConfig, compute_momentum_baseline — Step F PR3) + Low Volatility baseline 평가 (LowVolBaselineConfig, compute_low_volatility_baseline — Step F PR4) + RSI 평균회귀 baseline 평가 (RSIMRBaselineConfig, compute_rsi_mr_baseline — Step F PR5) + RSI MR sensitivity grid (RSIMRParameterAxis, RSIMRSensitivityGrid, RSIMRSensitivityRow, step_f_rsi_mr_grid, run_rsi_mr_sensitivity, run_rsi_mr_sensitivity_combos, run_rsi_mr_sensitivity_parallel, render_markdown_table, write_csv, append_sensitivity_row, load_sensitivity_rows, load_completed_combos, filter_remaining_combos, merge_sensitivity_rows — ADR-0023 C4)
...
- 포매터·린터: `ruff` (lint + format). black 폐기 (ADR-0026, 2026-05-03). `pre-commit` 훅 적용.
- 로깅: `loguru` (구조화 로그 권장).
...
- **정적 검사 4종** (`pytest` · `ruff check` · `ruff format --check` · `pyright`) — 읽기 전용, 서로 영향 0. 항상 `run_in_background: true` 로 동시 기동.
...
- 사용자에게 장시간 작업을 띄울 때는 어떤 작업이 돌고 있는지 한 줄로 알린다 — "pytest + ruff check + ruff format --check + pyright 4종 병렬 실행 중".
...
| [`ci-lint-full-scope.sh`](./.claude/hooks/ci-lint-full-scope.sh) | PreToolUse / `Bash` | `git push`(—dry-run 제외) 직전 CI 2 종 lint (`uv run ruff check` + `uv run ruff format --check`, 모두 `src scripts tests`) 중 하나라도 실패. CI "Lint, format, test" job 과 동일 범위 강제 (PR #43 UP037 재발 방지, ADR-0026 black 폐기) | `STOCK_AGENT_LINT_BYPASS=1 git push ...` (24시간 내 원인 제거 필수) |
...
- **복구 로드맵 진행 상황**:
  ...
  - **Step E** — 전략 교체 (VWAP mean-reversion / opening gap reversal). A~D 전원 실패 전제. **FAIL 종료 (2026-05-01, ADR-0021)**. PR1~PR4 코드 산출물 모두 완료 후 4 런 실행 결과 전원 ADR-0019 게이트 미통과.
    - VWAP-MR Top 50: MDD -49.09% · 승×손익비 0.046 · 샤프 -11.02 — FAIL.
    - VWAP-MR Top 100: MDD -50.11% · 승×손익비 0.045 · 샤프 -10.35 — FAIL.
...
- **Phase 3 진입 게이팅 해제 (ADR-0019 + ADR-0023, 2026-05-03)**: ADR-0023 의 C1~C4 추가 검증 전원 통과 → Phase 2 PASS 공식 선언 + Phase 3 착수 재허가. `main.py` 모의투자 무중단 운영 착수 가능. `execution/`·`main.py`·`monitor/`·`storage/` 코드 산출물 즉시 재사용 가능 — PR5 가 `Strategy` Protocol 준수.

...
- 알림: 텔레그램 봇
- 스택: Python 3.11+, `uv`, `python-kis 2.x`, `pykrx`, `pyyaml`, `APScheduler`, `loguru`, `python-telegram-bot`, SQLite (백테스트 엔진은 자체 시뮬레이션 루프 — `backtesting.py` 폐기 결정 2026-04-20)
...
- [src/stock_agent/data/CLAUDE.md](./src/stock_agent/data/CLAUDE.md) — 과거/실시간 시세 수집 모듈 (HistoricalDataStore, DailyBar, SQLite 캐시)
...
- [src/stock_agent/backtest/CLAUDE.md](./src/stock_agent/backtest/CLAUDE.md) — 백테스트 엔진 모듈 (BacktestEngine, BacktestConfig, BacktestResult, BacktestMetrics, TradeRecord, DailyEquity, BarLoader, InMemoryBarLoader; 자체 시뮬레이션 루프, 한국 시장 비용 반영)
...
- **Phase 2 진행 중 — ORB 전략 엔진 + 리스크 매니저 + 백테스트 엔진 코어 완료** (2026-04-20)
  ...
  - pytest **245 → 324건 green** (test_backtest_engine 79 추가 — costs 18 + metrics 22 + loader 8 + engine 31). 회귀 없음. 의존성 추가 없음.
  - 미완료: `scripts/backtest.py` CLI, 실데이터 분봉 어댑터(KIS 과거 분봉 API or CSV 임포트), 파라미터 민감도 리포트, 2~3년 실데이터 백테스트 PASS 검증(MDD < -15%). Phase 2 전체 PASS 선언은 이후.

- **다음 작업**
  - **Phase 2 잔여 (후속 PR)**: `scripts/backtest.py` CLI · 실데이터 분봉 어댑터(KIS 과거 분봉 API or CSV 임포트) · 파라미터 민감도 리포트 · 2~3년 실데이터 백테스트 PASS 검증(MDD < -15%). (`scripts/backtest.py` 는 아직 미존재 — "실행 가능" 아님.)
  - **Phase 3 착수 전 필수 (별도 진행)**: 운영자 `.env` 실전 키 3종(`KIS_LIVE_APP_KEY`, `KIS_LIVE_APP_SECRET`, `KIS_LIVE_ACCOUNT_NO`) 기입 + KIS Developers 포털 IP 화이트리스트 등록 + `uv run python scripts/healthcheck.py` 4종(삼성전자 현재가 포함) 통과 확인.

...
- 알림: 텔레그램 봇
- 스택: Python 3.11+, `uv`, `python-kis 2.x`, `pykrx`, `pyyaml`, `backtesting.py`, `APScheduler`, `loguru`, `python-telegram-bot`, SQLite
...
- **Phase 2 진행 중 — ORB 전략 엔진 첫 산출물 완료** (2026-04-20)
  ...
  - pytest **167건 green** (기존 131 + 신규 36 — test_strategy_orb). 회귀 없음. 의존성 추가 없음.
  - 미완료: `risk/manager.py`, `backtest/engine.py`, 파라미터 민감도 리포트. Phase 2 전체 PASS 선언은 이후.

- **다음 작업**
  - **Phase 2 계속**: `risk/manager.py` (포지션 사이징, 손절/익절, 일일 손실 한도) → `backtest/engine.py` (backtesting.py 래퍼, 최근 2~3년 분봉 백테스트) → 파라미터 민감도 리포트.
